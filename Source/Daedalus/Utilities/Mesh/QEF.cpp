...
namespace utils {
	Vector3D<> QEF::evaluate(double mat[][3], double * vec, const Int32 rows) {
		...
		return Vector3D<>((float)x[0], (float)x[1], (float)x[2]);
	}
		...
		const Int32 rows)
	{
		...
		for (Int32 i = 0; i < 3; ++i) {
			...
			// through that column
			double *ptrs[MAXROWS];
			Int32 num_ptrs = rows - i;
			for (Int32 q = 0; q < num_ptrs; ++q)
				ptrs[q] = &mat[q + i][i];
			...
			// all computations below this poconst Int32 are performed
	...
	double QEF::factorize_hh(double *ptrs[], const Int32 n)
	{
		double tau = 0.0;
	...
	void QEF::chop(double *a, double *b, const Int32 n)
	{
		double ai = a[0];
